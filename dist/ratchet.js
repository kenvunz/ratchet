...
!function () {
  ...
  var PUSH = function (options) {
    ...
    xhr.onreadystatechange = function () {
      ...
      var status = /^http/.test( location.protocol )? 200 : 0
      if (xhr.readyState == 4) xhr.status == status ? success(xhr, options) : failure(options.url);
    };
  ...
  var swapContent = function (swap, container, transition, complete) {
    ...
    if (!transition) {
      ...
      else if (swap.classList.contains('content')) wrapper.appendChild(swap);
      else wrapper.insertBefore(swap, document.querySelector('.content'));
    } else {
